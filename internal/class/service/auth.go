...
func (s *AuthService) GetJwtUserID(jwtToken string) (*model2.ContextUserID, error) {
	token, err := jwt.Parse(
		jwtToken,
...
func (s *AuthService) GetJwtUserRole(jwtToken string) (*model2.ContextUserRole, error) {
	token, err := jwt.Parse(
		jwtToken,
		func(token *jwt.Token) (interface{}, error) {
			if _, ok := token.Method.(*jwt.SigningMethodHMAC); !ok {
...
func (s *AuthService) getUserIDFromJwt(claims jwt.MapClaims) (*model2.ContextUserID, error) {
	user := &model2.ContextUserID{}
...
func (s *AuthService) getUserRoleFromJwt(claims jwt.MapClaims) (*model2.ContextUserRole, error) {
	user := &model2.ContextUserRole{}
	role, ok := claims["role"]
	if !ok {
		return nil, fmt.Errorf("user is not exists in jwt")
	}
